...
local Helpers = require('nvim-ssns.formatter.passes.04_structure_helpers')

-- Import constants and helpers
local MAJOR_CLAUSES = Helpers.MAJOR_CLAUSES
local CLAUSE_NEWLINE_CONFIG = Helpers.CLAUSE_NEWLINE_CONFIG
local JOIN_MODIFIERS = Helpers.JOIN_MODIFIERS
local get_upper = Helpers.get_upper
local is_and_or = Helpers.is_and_or
local is_comma = Helpers.is_comma
...
function StructurePass.run(tokens, config)
  ...
  -- State tracking
  local state = Helpers.create_state()
  ...
  for i, token in ipairs(tokens) do
    ...
    if is_comma(token) then
      local add_newline, next_indent = Helpers.process_comma(token, state, config, tokens, i)
      if add_newline then
        token.trailing_newline = true
        state.pending_stacked_newline = true
    ...
    Helpers.apply_pending_first_item_newlines(token, state, config)
    ...
    if token.type == "semicolon" or token.type == "go" then
      Helpers.reset_state_on_statement_end(state)
    end
  end
