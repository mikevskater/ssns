...
local FormatterContext = require("ssns.formatter.context")
...
local function apply_token_casing(token, config)
  -- If casing pass already applied, use pre-cased text for keywords
  if token.casing_applied then
    return token.text
  end
...
function Output.generate(tokens, config)
  ...
  -- FormatterContext holds all clause tracking, pending flags, and state
  local ctx = FormatterContext:new()

  -- Local aliases for frequently accessed context properties (for backward compatibility)
  -- These reference the context tables directly
  local in_flags = ctx.in_flags
  local pending = ctx.pending
  local paren_depths = ctx.paren_depths
  local state = ctx.state

  -- Comprehensive state reset function for statement/batch boundaries
  local function reset_all_state()
    ctx:reset_clause_state()
  end
  ...
  for i, token in ipairs(tokens) do
    ...
    local skip_space_before = state.line_just_started  -- Skip space if we just added indent
    state.line_just_started = false  -- Reset the flag
    ...
    if pending.stacked_indent and not token.is_comment then
    ...
    if pending.where_stacked_indent and not token.is_comment then
      -- First condition after WHERE - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.where_stacked_indent = false
    end

    -- Phase 1: Handle pending from_table_style stacked_indent newline (first table after FROM)
    if pending.from_stacked_indent and not token.is_comment then
      -- First table after FROM - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.from_stacked_indent = false
    end

    -- Phase 1: Handle pending on_condition_style stacked_indent newline (first condition after ON)
    if pending.on_stacked_indent and not token.is_comment then
      -- First condition after ON - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.on_stacked_indent = false
    end

    -- Phase 4: Handle pending in_list_style stacked_indent newline (first value after IN ()
    if pending.in_stacked_indent and not token.is_comment and token.type ~= "paren_open" then
      -- First value after IN ( - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.in_stacked_indent = false
    end

    -- Phase 2: Handle pending insert_columns_style stacked_indent newline (first column after INSERT (...))
    if pending.insert_columns_stacked_indent and not token.is_comment and token.type ~= "paren_close" then
      -- First column after INSERT ( - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.insert_columns_stacked_indent = false
    end

    -- Phase 2: Handle pending insert_values_style stacked_indent newline (first value after VALUES (...))
    if pending.values_stacked_indent and not token.is_comment and token.type ~= "paren_close" then
      -- First value after VALUES ( - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.values_stacked_indent = false
    end

    -- Phase 2: Handle pending cte_columns_style stacked_indent newline (first column after CTE name (...))
    if pending.cte_columns_stacked_indent and not token.is_comment and token.type ~= "paren_close" then
      -- First column after CTE name ( - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.cte_columns_stacked_indent = false
    end

    -- Phase 1: Handle pending where_between_style stacked_indent newline (first value after BETWEEN)
    if pending.between_stacked_indent and not token.is_comment then
      -- First value after BETWEEN - add newline
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
      pending.between_stacked_indent = false
    end

    -- Phase 4: Handle pending function_arg_style stacked_indent newline (first arg after function ()
    if pending.function_stacked_indent and not token.is_comment and token.type ~= "paren_close" then
      -- First arg after function ( - add newline
      needs_newline = true
      if #ctx.function_call_stack > 0 then
        current_indent = ctx.function_call_stack[#ctx.function_call_stack].base_indent or 0
      end
      extra_indent = 1
      pending.function_stacked_indent = false
    end
    ...
    if token.type == "keyword" then
      ...
      local function reset_clauses()
        in_flags.select_list = false
        in_flags.from_clause = false
        in_flags.where_clause = false
        in_flags.join_clause = false
        in_flags.on_clause = false
        in_flags.group_by_clause = false
        in_flags.order_by_clause = false
        in_flags.having_clause = false
        in_flags.set_clause = false
        in_flags.values_clause = false
        in_flags.insert_columns = false
        in_flags.insert_columns_paren = false
        paren_depths.insert_columns = 0
        pending.insert_columns_stacked_indent = false
        pending.join = false
        pending.where_stacked_indent = false
        pending.from_stacked_indent = false
      ...
      elseif upper == "SELECT" then
        ...
        if in_flags.cte and (token.paren_depth or 0) == 0 then
          in_flags.cte = false
        end
        reset_clauses()
        in_flags.select_list = true
      elseif upper == "FROM" then
        -- Flush SET alignment buffer if active (UPDATE ... SET ... FROM)
        if state.set_align_active then
          ...
          if line_text:match("%S") then
            ...
            if col_part then
              ...
              if #col_part > state.set_align_max_col_width then
                state.set_align_max_col_width = #col_part
              end
              table.insert(state.set_align_buffer, {
          ...
          if #state.set_align_buffer > 0 then
            flush_set_align_buffer(state.set_align_buffer, result, state.set_align_max_col_width, config)
          end
          current_line = {}  -- Clear current line since we handled it
          skip_space_before = true  -- Skip leading space for FROM
          state.set_align_active = false
          state.set_align_buffer = {}
          state.set_align_current = nil
          in_flags.update_statement = false
        end
        reset_clauses()
        in_flags.from_clause = true
        ...
        if from_table_style == "stacked_indent" then
          pending.from_stacked_indent = true
        end
      elseif upper == "WHERE" then
        ...
        if where_style == "stacked_indent" then
          pending.where_stacked_indent = true
        end
      elseif upper == "GROUP" then
        reset_clauses()
        in_flags.group_by_clause = true
      elseif upper == "ORDER" and not token.in_over_clause then
        reset_clauses()
        in_flags.order_by_clause = true
      elseif upper == "HAVING" then
        reset_clauses()
        in_flags.having_clause = true
      elseif upper == "SET" then
        reset_clauses()
        in_flags.set_clause = true
        -- Activate SET alignment buffering if enabled and in UPDATE statement with stacked style
        if in_flags.update_statement and config.update_set_align and config.update_set_style == "stacked" then
          state.set_align_active = true
          state.set_align_buffer = {}
          state.set_align_current = { column_name = "", value_tokens = {}, seen_equals = false }
          state.set_align_max_col_width = 0
        end
      elseif upper == "VALUES" then
        reset_clauses()
        in_flags.values_clause = true
      elseif upper == "ON" and in_flags.join_clause then
        in_flags.on_clause = true
        ...
        if on_cond_style == "stacked_indent" then
          pending.on_stacked_indent = true
        end
      elseif upper == "MERGE" then
        ...
        reset_clauses()
        in_flags.merge_statement = true
      elseif upper == "UPDATE" then
        if in_flags.cte and (token.paren_depth or 0) == 0 then
          in_flags.cte = false
        end
        reset_clauses()
        in_flags.update_statement = true
      elseif upper == "INSERT" then
        if in_flags.cte and (token.paren_depth or 0) == 0 then
          in_flags.cte = false
        end
        reset_clauses()
        in_flags.insert_columns = true
      ...
      elseif upper == "CREATE" then
        ...
        pending.create = true
        reset_clauses()
      elseif upper == "TABLE" and pending.create then
        -- CREATE TABLE detected - will enter column definitions at next open paren
        in_flags.create_table = true
        pending.create = false
      elseif pending.create then
    ...
    -- Use structure pass annotation if available (token.needs_newline_before)
    if token.needs_newline_before then
      needs_newline = true
      current_indent = token.clause_indent or token.indent_level or 0
      if token.add_empty_line_before then
        add_empty_line_after_flush = true
      end
    elseif config.newline_before_clause and token.type == "keyword" then
      local upper = string.upper(token.text)
      ...
      elseif is_apply_modifier(upper) then
        ...
        if next_is_apply then
          ...
          if not pending.join then
            needs_newline = true
            current_indent = base_indent
            state.join_modifiers = {}
            if config.empty_line_before_join then
              add_empty_line_after_flush = true
            end
          end
          pending.join = true
          table.insert(state.join_modifiers, upper)
    ...
    if token.type == "keyword" then
      ...
      if upper == "DISTINCT" and in_flags.select_list and config.select_distinct_newline then
        needs_newline = true
        current_indent = base_indent
        extra_indent = 1
      end
      ...
      if upper == "TOP" and in_flags.select_list and config.select_top_newline then
        needs_newline = true
        current_indent = base_indent
        extra_indent = 1
      end
      ...
      if upper == "INTO" and in_flags.select_list and config.select_into_newline then
        needs_newline = true
        current_indent = base_indent
      end
    end
    ...
    -- Skip if this AND belongs to BETWEEN - BETWEEN AND is handled separately by where_between_style
    -- Use token.is_between_and annotation from expressions pass if available
    local where_style = config.where_condition_style or "stacked"
    local is_between_and = token.is_between_and or in_flags.between_clause
    if in_flags.where_clause and is_and_or(token) and where_style ~= "inline" and not is_between_and then
    ...
    local is_between_and_token = token.is_between_and or (in_flags.between_clause and token.type == "keyword" and string.upper(token.text) == "AND")
    if is_between_and_token and between_style ~= "inline" then
    ...
    local on_cond_style = config.on_condition_style or "inline"
    if in_flags.on_clause and is_and_or(token) and on_cond_style ~= "inline" then
      -- stacked and stacked_indent both stack conditions
    ...
    if config.boolean_operator_newline and is_and_or(token) and not in_flags.where_clause and not in_flags.on_clause then
      needs_newline = true
      current_indent = token.indent_level or 0
      extra_indent = 1
    end
    ...
    if token.type == "comma" and in_flags.select_list then
    ...
    if token.type == "comma" and in_flags.from_clause then
    ...
    if token.type == "comma" and in_flags.set_clause then
    ...
    if needs_newline and #current_line > 0 then
      ...
      -- Use spacing pass annotation if available, fallback to runtime check
      local should_space = token.space_before
      if should_space == nil then
        should_space = needs_space_before(prev_token, token, config)
      end
      if not skip_space_before and should_space then
        table.insert(current_line, " ")
      end
    end
    ...
    if is_join_keyword(string.upper(token.text)) and join_style == "full" then
      ...
      for _, mod in ipairs(state.join_modifiers) do
      ...
      if #state.join_modifiers == 0 then
      ...
      -- Reset state.join_modifiers after processing JOIN
      state.join_modifiers = {}
    end
    ...
    if token.needs_from_keyword and config.delete_from_keyword then
      ...
      if config.delete_from_newline then
        ...
        state.line_just_started = false  -- We added content, not just indent
      else
    ...
    local is_between_and_trailing = token.is_between_and or in_flags.between_clause
    if in_flags.where_clause and is_and_or(token) and where_cond_style ~= "inline" and config.and_or_position == "trailing" and not is_between_and_trailing then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
      ...
      state.line_just_started = true  -- Skip space before next token
    end
    ...
    if in_flags.on_clause and is_and_or(token) and on_cond_style_trailing ~= "inline" and config.on_and_position == "trailing" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    -- Note: stacked_indent also stacks columns (it's stacked with first on new line)
    if token.type == "comma" and in_flags.select_list and (select_list_style == "stacked" or select_list_style == "stacked_indent") and config.comma_position == "trailing" and paren_depth == 0 then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    local from_style = config.from_table_style or "inline"
    -- Note: stacked_indent also stacks tables (it's stacked with first on new line)
    if token.type == "comma" and in_flags.from_clause and (from_style == "stacked" or from_style == "stacked_indent") and config.comma_position == "trailing" and paren_depth == 0 then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if token.type == "comma" and in_flags.set_clause and config.update_set_style == "stacked" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
    ...
    if token.type == "comma" and in_flags.group_by_clause and config.group_by_style == "stacked" and paren_depth == 0 then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if token.type == "comma" and in_flags.order_by_clause and config.order_by_style == "stacked" and paren_depth == 0 then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if token.type == "comma" and in_flags.values_clause and config.insert_multi_row_style == "stacked" and paren_depth == 0 then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if in_flags.create_table then
      if token.type == "paren_open" then
        paren_depths.create_table = paren_depths.create_table + 1
      elseif token.type == "paren_close" then
        paren_depths.create_table = paren_depths.create_table - 1
        if paren_depths.create_table <= 0 then
          -- Exiting CREATE TABLE column definitions
          in_flags.create_table = false
          paren_depths.create_table = 0
        end
      end
    end

    -- Phase 4: create_table_column_newline - each column definition on new line
    if token.type == "comma" and in_flags.create_table and paren_depths.create_table == 1 and config.create_table_column_newline ~= false then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if pending.in_keyword and token.type == "paren_open" then
      pending.in_keyword = false
      in_flags.in_clause = true
      paren_depths.in_clause = 1
      ...
      if in_style == "stacked_indent" then
        pending.in_stacked_indent = true
      end
    elseif in_flags.in_clause then
      if token.type == "paren_open" then
        paren_depths.in_clause = paren_depths.in_clause + 1
      elseif token.type == "paren_close" then
        paren_depths.in_clause = paren_depths.in_clause - 1
        if paren_depths.in_clause <= 0 then
          in_flags.in_clause = false
          paren_depths.in_clause = 0
          pending.in_stacked_indent = false
        end
      end
    end

    -- Reset pending.in_keyword if we see something other than paren_open after IN
    if pending.in_keyword and token.type ~= "paren_open" and token.type ~= "whitespace" then
      pending.in_keyword = false
    end

    -- Track IN clause for in_list_style (AFTER the reset check above)
    if token.type == "keyword" and string.upper(token.text) == "IN" then
      pending.in_keyword = true
    end

    -- Phase 4: in_list_style - each value in IN clause on new line
    local in_style = config.in_list_style or config.where_in_list_style or "inline"
    if token.type == "comma" and in_flags.in_clause and paren_depths.in_clause == 1 and in_style ~= "inline" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if in_flags.insert_columns and not in_flags.insert_columns_paren and token.type == "paren_open" then
      -- Entering column list parentheses
      in_flags.insert_columns_paren = true
      paren_depths.insert_columns = 1

      -- Check for stacked_indent - need newline after opening paren
      local insert_col_style = config.insert_columns_style or "inline"
      if insert_col_style == "stacked_indent" then
        pending.insert_columns_stacked_indent = true
      end
    elseif in_flags.insert_columns_paren then
      if token.type == "paren_open" then
        paren_depths.insert_columns = paren_depths.insert_columns + 1
      elseif token.type == "paren_close" then
        paren_depths.insert_columns = paren_depths.insert_columns - 1
        if paren_depths.insert_columns <= 0 then
          -- Exiting INSERT column list
          in_flags.insert_columns_paren = false
          in_flags.insert_columns = false  -- No longer in INSERT context after column list
          paren_depths.insert_columns = 0
          pending.insert_columns_stacked_indent = false
        end
      end
    ...
    local insert_col_style = config.insert_columns_style or "inline"
    if token.type == "comma" and in_flags.insert_columns_paren and paren_depths.insert_columns == 1 and insert_col_style ~= "inline" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if in_flags.values_clause and not in_flags.values_paren and token.type == "paren_open" then
      -- Entering VALUES parentheses
      in_flags.values_paren = true
      paren_depths.values = 1

      -- Check for stacked_indent - need newline after opening paren
      local values_style = config.insert_values_style or "inline"
      if values_style == "stacked_indent" then
        pending.values_stacked_indent = true
      end
    elseif in_flags.values_paren then
      if token.type == "paren_open" then
        paren_depths.values = paren_depths.values + 1
      elseif token.type == "paren_close" then
        paren_depths.values = paren_depths.values - 1
        if paren_depths.values <= 0 then
          -- Exiting VALUES value list
          in_flags.values_paren = false
          paren_depths.values = 0
          pending.values_stacked_indent = false
          -- Don't reset in_flags.values_clause here - might have more value rows
        end
      end
    end
    ...
    local values_style = config.insert_values_style or "inline"
    if token.type == "comma" and in_flags.values_paren and paren_depths.values == 1 and values_style ~= "inline" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if token.is_cte_name then
      in_flags.cte_columns = true  -- Expecting optional column list or AS
    end

    -- AS keyword ends CTE column context (whether or not there was a column list)
    if token.is_cte_as and in_flags.cte_columns then
      in_flags.cte_columns = false
      in_flags.cte_columns_paren = false
      paren_depths.cte_columns = 0
      pending.cte_columns_stacked_indent = false
    end

    -- Track when we enter CTE column list parentheses
    if in_flags.cte_columns and not in_flags.cte_columns_paren and token.type == "paren_open" then
      -- Entering CTE column list parentheses
      in_flags.cte_columns_paren = true
      paren_depths.cte_columns = 1

      -- Check for stacked_indent - need newline after opening paren
      local cte_col_style = config.cte_columns_style or "inline"
      if cte_col_style == "stacked_indent" then
        pending.cte_columns_stacked_indent = true
      end
    elseif in_flags.cte_columns_paren then
      if token.type == "paren_open" then
        paren_depths.cte_columns = paren_depths.cte_columns + 1
      elseif token.type == "paren_close" then
        paren_depths.cte_columns = paren_depths.cte_columns - 1
        if paren_depths.cte_columns <= 0 then
          -- Exiting CTE column list (but still in_flags.cte_columns until AS)
          in_flags.cte_columns_paren = false
          paren_depths.cte_columns = 0
          pending.cte_columns_stacked_indent = false
        end
    ...
    local cte_col_style = config.cte_columns_style or "inline"
    if token.type == "comma" and in_flags.cte_columns_paren and paren_depths.cte_columns == 1 and cte_col_style ~= "inline" then
      local line_text = table.concat(current_line, "")
      if line_text:match("%S") then
        table.insert(result, line_text)
      end
    ...
    if in_flags.between_clause and token.type == "keyword" and string.upper(token.text) == "AND" then
      -- After AND in BETWEEN, we exit the BETWEEN clause (BETWEEN x AND y is complete)
      in_flags.between_clause = false
      state.between_value_count = 0
      pending.between_stacked_indent = false
    end
    ...
    if not in_flags.between_clause then
    ...
    if pending.function_call and token.type == "paren_open" then
      pending.function_call = false
      table.insert(ctx.function_call_stack, {
        paren_depth = 1,
      ...
      if func_style == "stacked_indent" then
        pending.function_stacked_indent = true
      end
    elseif #ctx.function_call_stack > 0 then
      -- Track parenthesis depth within function calls
      local current_func = ctx.function_call_stack[#ctx.function_call_stack]
      if token.type == "paren_open" then
      ...
      elseif token.type == "paren_close" then
        ...
        if current_func.paren_depth <= 0 then
          ...
          table.remove(ctx.function_call_stack)
        end
    ...
    -- Reset pending.function_call if we see something other than paren_open
    if pending.function_call and token.type ~= "paren_open" and token.type ~= "whitespace" then
      pending.function_call = false
    end

    -- Detect function keyword and set pending flag (AFTER reset check so it persists to next token)
    if token.keyword_category == "function" then
      pending.function_call = true
    end

    -- Phase 4: function_arg_style - handle comma in function calls for stacked style
    local func_style = config.function_arg_style or "inline"
    if token.type == "comma" and #ctx.function_call_stack > 0 and func_style ~= "inline" then
      local current_func = ctx.function_call_stack[#ctx.function_call_stack]
    ...
    if token.type == "semicolon" then
      ...
      state.line_just_started = true  -- Next statement starts fresh, no leading space
    end
    ...
    if token.type == "go" then
      ...
      if #current_line > 1 then
        ...
        local line_text = table.concat(current_line, "")
        -- Trim trailing whitespace (space before GO that was added by spacing logic)
        line_text = line_text:gsub("%s+$", "")
        if line_text:match("%S") then
          table.insert(result, line_text)
        end
    ...
    -- Reset pending.join if we hit something other than OUTER or JOIN
    if token.type == "keyword" then
      local upper = string.upper(token.text)
      if pending.join and upper ~= "OUTER" and upper ~= "JOIN" and not is_join_modifier(upper) then
        pending.join = false
      end
    end
