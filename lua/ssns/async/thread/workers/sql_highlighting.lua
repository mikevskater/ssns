-- SQL Highlighting Worker
-- Performs tokenization and basic classification in a separate thread
-- Pure Lua only - NO vim.* APIs
--
-- Input: { text: string, options: table? }
-- Output: classified tokens with highlight groups
--
...
local batch_interval_ms = options.batch_interval_ms or 50
...
if text == "" then
  send({ type = "complete", result = { tokens = {} } })
  return
end
...
local function emit_token(force_type)
  ...
  if token_type == TOKEN_TYPE.KEYWORD or token_type == TOKEN_TYPE.GO then
    local category = keyword_category or "misc"
    highlight_group = HIGHLIGHT_MAP["keyword_" .. category]
  elseif token_type == TOKEN_TYPE.STRING then
    highlight_group = HIGHLIGHT_MAP.string
  elseif token_type == TOKEN_TYPE.NUMBER then
    highlight_group = HIGHLIGHT_MAP.number
  elseif token_type == TOKEN_TYPE.OPERATOR then
    highlight_group = HIGHLIGHT_MAP.operator
  elseif token_type == TOKEN_TYPE.COMMENT then
    highlight_group = HIGHLIGHT_MAP.comment
  elseif token_type == TOKEN_TYPE.LINE_COMMENT then
    highlight_group = HIGHLIGHT_MAP.line_comment
  elseif token_type == TOKEN_TYPE.VARIABLE then
    highlight_group = HIGHLIGHT_MAP.variable
  elseif token_type == TOKEN_TYPE.GLOBAL_VARIABLE then
    highlight_group = HIGHLIGHT_MAP.global_variable
  elseif token_type == TOKEN_TYPE.SYSTEM_PROCEDURE then
    highlight_group = HIGHLIGHT_MAP.system_procedure
  elseif token_type == TOKEN_TYPE.TEMP_TABLE then
